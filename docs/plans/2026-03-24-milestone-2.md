...
> ✅ **完成** commit `008c895`。索引已在 `001_init.up.sql` migration 中存在，無需新增 migration 檔。
...
> ✅ **完成** commit `50d4293`。
> **實作調整：** 原 plan 用 `$1::DATE`、`$2::DATE`、`$3::BOOLEAN` positional params，sqlc 產出 `Column1/Column2/Column3` 非 nullable 型別（`time.Time`、`bool`）。`to` 為零值時 `wake_at <= 0001-01-01` 會過濾掉所有資料，因此改用 `sqlc.narg('from')`、`sqlc.narg('to')`、`sqlc.narg('abnormal_only')` 命名 nullable params，產出正確的 `From sql.NullTime`、`To sql.NullTime`、`AbnormalOnly sql.NullBool`。
...
> ✅ **完成** commit `b875cc9`。同 Task 1，改用 `sqlc.narg()` 命名 nullable params，產出 `From sql.NullTime`、`To sql.NullTime`。
...
> ✅ **完成** commit `0ce8ff6`。
> **實作調整：** plan 中的 `int16PtrToNullInt16Sleep` 改用已有的 `int16PtrToNullInt16`（同 package 可直接取用）。`nullInt32ToPtr` helper 定義在 `sleep_logs.go` 中。
...
> ✅ **完成** commit `57b3ed7`。全 16 test cases PASS（TestCreateSleepLog×6、TestListSleepLogs×5、TestUpdateSleepLog×6、TestDeleteSleepLog×3）。
