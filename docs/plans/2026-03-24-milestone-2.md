...
WHERE
    ($1::DATE IS NULL OR sleep_at::DATE >= $1::DATE)
    AND ($2::DATE IS NULL OR sleep_at::DATE <= $2::DATE)
...
func nullInt16ToInt16SleepPtr(n sql.NullInt16) *int16 {
    if !n.Valid {
        return nil
    }
    return &n.Int16
...
func int16PtrToNullInt16Sleep(i *int16) sql.NullInt16 {
    if i == nil {
...
func toSleepLogResponse(s sqlcdb.SleepLog) SleepLogResponse {
    return SleepLogResponse{
        ...
        DurationMin:  nullInt32ToInt32Ptr(s.DurationMin),
        AbnormalWake: s.AbnormalWake,
        Quality:      nullInt16ToInt16SleepPtr(s.Quality),
...
func ListSleepLogs(store SleepLogStore) gin.HandlerFunc {
    return func(c *gin.Context) {
        ...
        params := &sqlcdb.ListSleepLogsParams{Limit: 100}
...
func UpdateSleepLog(store SleepLogStore) gin.HandlerFunc {
    return func(c *gin.Context) {
        ...
        if req.SleepAt != nil && req.WakeAt != nil && !req.WakeAt.After(*req.SleepAt) {
            c.JSON(http.StatusBadRequest, errResponse("VALIDATION_ERROR", "wake_at must be after sleep_at"))
            return
        }
        if err := validate.Struct(req); err != nil {
        ...
        updated, err := store.UpdateSleepLog(c.Request.Context(), &sqlcdb.UpdateSleepLogParams{
            ID:      id,
            SleepAt: sleepAtNull,
            WakeAt:  wakeAtNull,
            Quality: int16PtrToNullInt16Sleep(req.Quality),
...
func nullInt32ToInt32ActivityPtr(n sql.NullInt32) *int32 {
    if !n.Valid {
        return nil
    }
    return &n.Int32
...
func toDailyActivityResponse(a sqlcdb.DailyActivity) DailyActivityResponse {
    return DailyActivityResponse{
        ...
        Steps:          nullInt32ToInt32ActivityPtr(a.Steps),
...
func ListDailyActivities(store DailyActivityStore) gin.HandlerFunc {
    return func(c *gin.Context) {
        ...
        params := &sqlcdb.ListDailyActivitiesParams{Limit: 100}
...
func TestDailyActivityUniqueConstraint(t *testing.T) {
    ...
    errStr := err.Error()
    if !containsStr(errStr, "23505") && !containsStr(errStr, "unique") {
...
### Step 2: 查閱 layerchart 可用元件

在實作前先確認 layerchart 暴露的 API：
...
若 layerchart 暴露 `Chart`、`Line`、`XAxis`、`YAxis`、`Tooltip` 等低階元件，使用方案 A（重構為低階 API）。否則使用方案 B（在 `<LineChart>` 下方加一個對齊的 SVG 標記列）。
